...
The Calendar uses green shading for recommendation points and red shading for recurring traffic conditions derived directly from measurements. A split green/red cell contains both signals. Red shading appears only when a segment-local month, weekday, and hour has at least two observations and its average delay exceeds the meaningful slowdown threshold. Hovering the day shows the affected segments, local times, delays, and sample counts.
