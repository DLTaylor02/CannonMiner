...
final class Simulator
{
    ...
    private function startSegment(array &$state,string $name,DateTimeImmutable $at,string $id,int $seed):string
    {
        ...
        $conditions=$this->traffic->conditions($segment,$at,$seed+count($state['route']));$speed=max(20,(float)$state['vehicle']['target_speed']);$obstacleSeed=$seed+count($state['route'])*17;$obstacle=$this->obstacle($obstacleSeed,$driver,$copilot,$speed);$trafficCap=$trafficCapSeconds=null;$trafficTrigger=.15+(hexdec(substr(hash('sha256','traffic-position:'.$obstacleSeed),0,8))%401)/1000;
    ...
    private function advanceVehicle(array &$state,float $delta,string $id,DateTimeImmutable $at,string &$status,int $seed):void
    {
        ...
        if($segment['traffic_speed_cap']!==null&&($segment['traffic_started']??true)&&$segment['elapsed_seconds']>=$trafficEnd){$state['vehicle']['current_speed']=(float)$segment['cruising_speed'];if(!($segment['traffic_cap_lifted']??false)){$segment['traffic_cap_lifted']=true;$this->event($id,$at,'traffic_cap_lifted',['speed'=>round((float)$segment['cruising_speed'],1)]);}}
