...
final class Simulator
{
    ...
    public function act(string $id,int $userId,string $action,array $payload):void
    {
        ...
        if(in_array($state['pending_decision']['type']??'',['event','confirmation'],true)&&$action!=='acknowledge_event')throw new RuntimeException('Acknowledge the active event first.');
        ...
        elseif($action==='fuel'&&$status==='running'){$resumeSpeed=(float)$state['vehicle']['current_speed'];$deceleration=(int)ceil($resumeSpeed/5);$fuel=$this->fuelStop($state,$id,$at);$service=$fuel['seconds'];$acceleration=(int)ceil($resumeSpeed/5);$total=$deceleration+$service+$acceleration;$at=$at->modify('+'.$total.' seconds');$state['vehicle']['current_speed']=0;$status='paused';$state['pending_decision']=['type'=>'confirmation','title'=>'Fuel stop complete','message'=>sprintf('The vehicle slowed for %d seconds, dispensed %.1f gallons at %.1f GPM, spent 5 minutes paying, and needs %d seconds to return to speed. %d minutes %d seconds were added to the run.',$deceleration,$fuel['gallons'],$fuel['gpm'],$acceleration,intdiv($total,60),$total%60),'resume_speed'=>$resumeSpeed];}
        elseif($action==='copilot'){$this->assignCopilot($state,(string)($payload['copilot']??''),$id,$at);}
        elseif($action==='request_driver_change'&&$status==='running'){$seconds=(int)ceil(max(0,(float)$state['vehicle']['current_speed'])/5);$state['vehicle']['current_speed']=0;$at=$at->modify('+'.$seconds.' seconds');$status='awaiting_driver';$state['pending_decision']=['type'=>'driver','forced'=>false,'deceleration_seconds'=>$seconds];$this->event($id,$at,'driver_change_requested',['seconds'=>$seconds]);}
        elseif($action==='change_driver'&&$status==='awaiting_driver'){$deceleration=(int)($state['pending_decision']['deceleration_seconds']??0);$this->changeDriver($state,(string)($payload['driver']??''),$id,$at);$resumeSpeed=$this->segmentSpeed($state);$acceleration=(int)ceil($resumeSpeed/5);$service=120;$added=$service+$acceleration;$total=$deceleration+$added;$at=$at->modify('+'.$added.' seconds');$state['vehicle']['stopped_seconds']+=$service;$state['vehicle']['current_speed']=0;$status='paused';$state['pending_decision']=['type'=>'confirmation','title'=>'Driver change complete','message'=>sprintf('The vehicle slowed for %d seconds, the driver change took 120 seconds while stopped, and returning to speed takes %d seconds. %d minutes %d seconds were added to the run.',$deceleration,$acceleration,intdiv($total,60),$total%60),'resume_speed'=>$resumeSpeed];$this->event($id,$at,'driver_change_complete',['seconds'=>$total]);}
    ...
    private function startSegment(array &$state,string $name,DateTimeImmutable $at,string $id,int $seed):string
    {
        ...
        $state['current_segment']=$segment+['elapsed_seconds'=>0.0,'total_seconds'=>$drive+$delay,'cruising_speed'=>$speed,'traffic_speed_cap'=>$trafficCap,'traffic_cap_seconds'=>$trafficCapSeconds,'traffic_delay_seconds'=>$conditions['delay_seconds'],'obstacle_delay_seconds'=>$obstacle['delay'],'traffic_source'=>$conditions['source'],'traffic_samples'=>$conditions['samples'],'status_icon'=>$obstacle['icon'],'weather'=>$obstacle['weather']];
    ...
    {
        $segment=&$state['current_segment'];$state['vehicle']['current_speed']=$this->segmentSpeed($state);$remaining=max(0,$segment['total_seconds']-$segment['elapsed_seconds']);$used=min($delta,$remaining);$fraction=$segment['total_seconds']>0?$used/$segment['total_seconds']:1;
        ...
        if($segment['traffic_speed_cap']!==null&&$segment['elapsed_seconds']>=(float)$segment['traffic_cap_seconds'])$state['vehicle']['current_speed']=(float)$segment['cruising_speed'];
