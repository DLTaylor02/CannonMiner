...
final class Simulator
{
    ...
    private function startSegment(array &$state,string $name,DateTimeImmutable $at,string $id,int $seed):string
    {
        ...
        $conditions=$this->traffic->conditions($segment,$at,$seed+count($state['route']));$speed=max(20,(float)$state['vehicle']['target_speed']);$obstacleSeed=$seed+count($state['route'])*17;$obstacles=[];$windows=[[.15,.35],[.4,.6],[.65,.9]];foreach($windows as $index=>$window){$eventSeed=$obstacleSeed+$index*101;$obstacle=$this->obstacle($eventSeed,$driver,$copilot,$speed,$index===0);if(!$obstacle['type'])continue;$span=$window[1]-$window[0];$trigger=$window[0]+(hexdec(substr(hash('sha256','position:'.$eventSeed),0,8))%1001)/1000*$span;$obstacles[]=$obstacle+['trigger_fraction'=>$trigger];}$trafficCap=$trafficCapSeconds=null;$trafficTrigger=.15+(hexdec(substr(hash('sha256','traffic-position:'.$obstacleSeed),0,8))%401)/1000;
    ...
    {
        $segment=&$state['current_segment'];if(array_key_exists('pending_obstacles',$segment)){$obstacle=array_shift($segment['pending_obstacles']);}else{$obstacle=$segment['pending_obstacle'];$segment['pending_obstacle']=null;}$mapType=$obstacle['type']==='weather_event'?'weather_'.$obstacle['weather']:$obstacle['type'];$this->markMapEvent($state,$mapType);
    ...
    private function obstacle(int $seed,array $driver,?array $copilot,float $speed,bool $allowCrash):array
    {
