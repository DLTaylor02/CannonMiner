...
final class Simulator
{
    ...
    public function act(string $id,int $userId,string $action,array $payload):void
    {
        ...
        elseif($action==='fuel'&&$status==='running'){$resumeSpeed=(float)$state['vehicle']['current_speed'];$deceleration=(int)ceil($resumeSpeed/5);$this->markMapEvent($state,'fuel_stop');$fuel=$this->fuelStop($state,$id,$at);$service=$fuel['seconds'];$acceleration=(int)ceil($resumeSpeed/5);$total=$deceleration+$service+$acceleration;$at=$at->modify('+'.$total.' seconds');$state['vehicle']['current_speed']=0;$status='paused';$state['pending_decision']=['type'=>'confirmation','title'=>'Fuel stop complete','message'=>sprintf('The vehicle slowed for %d seconds, dispensed %.1f gallons at %.1f GPM, spent 5 minutes paying, and needs %d seconds to return to speed. %d minutes %d seconds were added to the run.',$deceleration,$fuel['gallons'],$fuel['gpm'],$acceleration,intdiv($total,60),$total%60),'resume_speed'=>$resumeSpeed,'fuel_stop'=>true];}
        ...
        elseif($action==='request_driver_change'&&$status==='running'){$seconds=(int)ceil(max(0,(float)$state['vehicle']['current_speed'])/5);$this->markMapEvent($state,'driver_change');$state['vehicle']['current_speed']=0;$at=$at->modify('+'.$seconds.' seconds');$status='awaiting_driver';$state['pending_decision']=['type'=>'driver','forced'=>false,'deceleration_seconds'=>$seconds,'map_event_marked'=>true];$this->event($id,$at,'driver_change_requested',['seconds'=>$seconds]);}
        elseif($action==='change_driver'&&$status==='awaiting_driver'){$duringFuel=(bool)($state['pending_decision']['fuel_stop']??false);$mapEventMarked=(bool)($state['pending_decision']['map_event_marked']??false);$deceleration=(int)($state['pending_decision']['deceleration_seconds']??0);$resumeSpeed=$duringFuel?(float)$state['pending_decision']['resume_speed']:$this->segmentSpeed($state);$this->changeDriver($state,(string)($payload['driver']??''),$id,$at);if(!$mapEventMarked)$this->markMapEvent($state,'driver_change');if($duringFuel){$state['vehicle']['current_speed']=$resumeSpeed;$status='running';$state['pending_decision']=null;$this->event($id,$at,'driver_change_complete',['seconds'=>0,'during_fuel'=>true]);}else{$acceleration=(int)ceil($resumeSpeed/5);$service=120;$added=$service+$acceleration;$total=$deceleration+$added;$at=$at->modify('+'.$added.' seconds');$state['vehicle']['stopped_seconds']+=$service;$state['vehicle']['current_speed']=0;$status='paused';$state['pending_decision']=['type'=>'confirmation','title'=>'Driver change complete','message'=>sprintf('The vehicle slowed for %d seconds, the driver change took 120 seconds while stopped, and returning to speed takes %d seconds. %d minutes %d seconds were added to the run.',$deceleration,$acceleration,intdiv($total,60),$total%60),'resume_speed'=>$resumeSpeed];$this->event($id,$at,'driver_change_complete',['seconds'=>$total]);}}
        elseif($action==='acknowledge_event'&&$status==='paused'&&in_array($state['pending_decision']['type']??'',['event','confirmation'],true)){$pending=$state['pending_decision'];$penalty=(int)($pending['penalty_seconds']??0);$stationary=(int)($pending['stationary_seconds']??0);if($penalty>0)$at=$at->modify('+'.$penalty.' seconds');$state['vehicle']['stopped_seconds']+=$stationary;$state['pending_decision']=null;$status='running';$state['vehicle']['current_speed']=isset($pending['resume_speed'])?(float)$pending['resume_speed']:$this->segmentSpeed($state);$this->event($id,$at,'event_acknowledged',['seconds'=>$penalty]);}
    ...
    private function startSegment(array &$state,string $name,DateTimeImmutable $at,string $id,int $seed):string
    {
        ...
        $conditions=$this->traffic->conditions($segment,$at,$seed+count($state['route']));$speed=max(20,(float)$state['vehicle']['target_speed']);$obstacle=$this->obstacle($seed+count($state['route'])*17,$driver,$copilot,$speed);if($obstacle['speed_cap']!==null)$speed=min($speed,$obstacle['speed_cap']);$trafficCap=$trafficCapSeconds=null;
        ...
        $drive=$segment['distance_miles']/$speed*3600+$conditions['delay_seconds'];$delay=in_array($obstacle['type'],['police_event','flat_tire'],true)?0.0:$obstacle['delay'];
        $state['current_segment']=$segment+['elapsed_seconds'=>0.0,'total_seconds'=>$drive+$delay,'speed_trace'=>[],'map_events'=>[],'cruising_speed'=>$speed,'traffic_speed_cap'=>$trafficCap,'traffic_cap_seconds'=>$trafficCapSeconds,'traffic_cap_lifted'=>false,'traffic_delay_seconds'=>$conditions['delay_seconds'],'obstacle_delay_seconds'=>$obstacle['delay'],'traffic_source'=>$conditions['source'],'traffic_samples'=>$conditions['samples'],'status_icon'=>$obstacle['icon'],'weather'=>$obstacle['weather']];
    ...
    private function markMapEvent(array &$state,string $type):void{if(!$state['current_segment'])return;$segment=&$state['current_segment'];$fraction=$segment['total_seconds']>0?max(0,min(1,$segment['elapsed_seconds']/$segment['total_seconds'])):0;$segment['map_events'][]=['type'=>$type,'fraction'=>$fraction];}
