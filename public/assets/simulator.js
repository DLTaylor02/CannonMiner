(() => {
  ...
  function renderDecision(state, status) {
    ...
    else if (pending.type === 'event') { const labels = {weather_event: 'Weather encountered', road_event: 'Road delay', police_event: 'Police stop', flat_tire: 'Flat tire', fuel_warning: 'Fuel below 25%'}; title.textContent = labels[pending.event] || 'Delay event'; if (pending.event === 'fuel_warning') copy.textContent = `Fuel has fallen to ${pending.percent}%. Decide whether to continue or stop for fuel after resuming.`; else if (pending.event === 'weather_event') copy.textContent = `${String(pending.weather).replace(/^./, letter => letter.toUpperCase())} limits the vehicle to ${pending.speed_cap} mph for this segment.`; else if (pending.event === 'flat_tire') copy.textContent = 'The vehicle will slow to a stop, spend 15 minutes replacing the tire, and accelerate back to speed.'; else if (pending.event === 'police_event') copy.textContent = 'The crew was released. The traffic stop adds 30 minutes to the run.'; else copy.textContent = `${pending.mitigated ? 'The crew reduced the impact. ' : ''}Expected delay: ${duration(pending.delay_seconds)}.`; options.innerHTML = '<button class="decision-ok" type="button" data-acknowledge>OK</button>'; options.querySelector('button').addEventListener('click', () => act('acknowledge_event')); }
