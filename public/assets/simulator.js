(() => {
  ...
  function render(simulation) {
    ...
    document.getElementById('simulation-location').textContent = segment ? `${locationName(segment.start)} to ${locationName(segment.end)}` : locationName(state.node); document.getElementById('simulation-traffic').textContent = segment ? `${segment.traffic_source} traffic${segment.traffic_speed_cap ? ` | ${Number(segment.traffic_speed_cap).toFixed(0)} mph cap` : ''} | ${Math.round(100 * segment.elapsed_seconds / Math.max(1, segment.total_seconds))}% of segment` : state.traffic_mode + ' traffic'; const weatherNode = document.getElementById('simulation-weather'), trafficIcon = document.getElementById('simulation-traffic-icon'); weatherNode.hidden = !segment?.status_icon; weatherNode.textContent = segment?.status_icon || ''; weatherNode.title = segment?.weather ? `${segment.weather} | ${segment.cruising_speed} mph maximum` : ''; trafficIcon.hidden = !segment?.traffic_speed_cap; trafficIcon.title = segment?.traffic_speed_cap ? `Traffic | ${Number(segment.traffic_speed_cap).toFixed(0)} mph maximum` : ''; drawMap(segment);
    document.getElementById('driver-cards').innerHTML = state.drivers.map(driver => { const role = driver.role === 'driver' && state.driver_copilot ? 'driver / co-pilot' : driver.role; return `<article class="driver-card"><header><strong>${escape(driver.name)}</strong><span>${escape(role.replace('_', ' '))}</span></header><small>END ${driver.endurance} | DRIVE ${driver.driving} | CO-PILOT ${driver.copilot}</small><div class="fatigue-track"><i style="width:${driver.fatigue}%;background:${driver.fatigue >= 75 ? 'var(--red)' : driver.fatigue >= 50 ? '#f0b429' : 'var(--green)'}"></i></div><small>${driver.fatigue.toFixed(1)}% fatigue${driver.locked_rest ? ' | REST REQUIRED' : ''}</small></article>`; }).join('');
    ...
    const ended = ['completed', 'failed'].includes(simulation.status), eventPending = (state.pending_decision?.type === 'event'), pause = document.getElementById('pause-action'); pause.textContent = simulation.status === 'paused' && !eventPending ? 'Resume' : 'Pause'; pause.disabled = ended || eventPending || !['running', 'paused'].includes(simulation.status); document.getElementById('driver-action').disabled = simulation.status !== 'running' || eventPending || state.drivers.length < 2; document.getElementById('fuel-action').disabled = ended || eventPending;
