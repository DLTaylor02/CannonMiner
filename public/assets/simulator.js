(() => {
  ...
  function drawMapEvents(target, points, project, events, lift = 0) {
    const stacked = new Map(), symbols = {fuel_stop: '\u26fd', driver_change: '\u21c4', police_event: '\u{1f693}', crash_event: '\u{1f4a5}', flat_tire: '\u{1f6de}', road_event: '\u{1f6a7}', obstacle_response: '\u2705', weather_fog: '\u{1f32b}\ufe0f', weather_rain: '\u{1f327}\ufe0f', weather_ice: '\u{1f9ca}', weather_snow: '\u{1f328}\ufe0f', weather_tail_wind: '\u{1f4a8}', weather_head_wind: '\u{1f32c}\ufe0f'};
  ...
  function renderDecision(state, status) {
    ...
    else if (pending.type === 'event') { const labels = {weather_event: 'Weather encountered', road_event: 'Trouble ahead', police_event: 'Lights in the mirror', flat_tire: 'That sound was expensive', fuel_warning: 'Running on fumes'}; title.textContent = labels[pending.event] || 'Delay event'; if (pending.event === 'fuel_warning') copy.textContent = `The fuel gauge has fallen to ${pending.percent}%. Portofino is still a long way off; decide whether to press on or stop for fuel after resuming.`; else if (pending.event === 'weather_event' && pending.weather === 'tail_wind') copy.textContent = 'The road gods are smiling. A tail wind improves fuel economy by 15% for the remainder of this segment.'; else if (pending.event === 'weather_event' && pending.weather === 'head_wind') copy.textContent = 'The wind has other plans. A head wind reduces fuel economy by 15% for the remainder of this segment.'; else if (pending.event === 'weather_event') copy.textContent = `${String(pending.weather).replace(/^./, letter => letter.toUpperCase())} has swallowed the road ahead. The vehicle is limited to ${pending.speed_cap} mph for the remainder of this segment.`; else if (pending.event === 'flat_tire') copy.textContent = 'A tire has given up the fight. The vehicle will slow to a stop, spend 15 minutes replacing it, and accelerate back to speed.'; else if (pending.event === 'police_event') copy.textContent = 'The crew caught a break and was released. The roadside conversation adds 30 minutes to the run.'; else copy.textContent = `${pending.mitigated ? 'Quick work by the crew softened the blow. ' : 'The road just collected its toll. '}Expected delay: ${duration(pending.delay_seconds)}.`; options.innerHTML = '<button class="decision-ok" type="button" data-acknowledge>OK</button>'; options.querySelector('button').addEventListener('click', () => act('acknowledge_event')); }
