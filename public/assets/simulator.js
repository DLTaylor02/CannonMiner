(() => {
  ...
  let highSpeedWarningShown = false; const speedWarning = document.getElementById('speed-warning'); document.querySelector('[data-close-speed-warning]').addEventListener('click', () => speedWarning.close());
  document.getElementById('speed-control').addEventListener('input', event => { clearTimeout(speedTimer); const speed = event.currentTarget.value; if (speed === '') return; const highSpeed = Number(speed) > 119; if (highSpeed && !highSpeedWarningShown && !speedWarning.open) speedWarning.showModal(); highSpeedWarningShown = highSpeed; speedTimer = setTimeout(() => act('target_speed', {speed}), 350); }); document.getElementById('copilot-control').addEventListener('change', event => act('copilot', {copilot: event.currentTarget.value})); document.getElementById('pause-action').addEventListener('click', () => act(currentStatus === 'paused' ? 'resume' : 'pause')); document.getElementById('driver-action').addEventListener('click', () => act('request_driver_change')); document.getElementById('fuel-action').addEventListener('click', () => act('fuel'));
