...
$app->get('/simulator',function(Request $request,Response $response)use($pdo,$render,&$identity):Response{
    $admin=in_array($identity['role'],['admin','superadmin'],true);$sql="SELECT s.id,s.user_id,u.username,s.status,s.mode,s.departure_at,s.simulated_at,s.state,s.created_at FROM simulations s JOIN users u ON u.id=s.user_id".($admin?'':' WHERE s.user_id=?')." ORDER BY CASE WHEN s.status NOT IN ('completed','failed') THEN 0 WHEN s.status='completed' AND s.state->>'node'='portofino' THEN 1 ELSE 2 END,CASE WHEN s.status='completed' AND s.state->>'node'='portofino' THEN NULLIF(s.state->>'finished_elapsed_seconds','')::double precision END ASC NULLS LAST,s.updated_at DESC LIMIT 100";
    $statement=$pdo->prepare($sql);$statement->execute($admin?[]:[$identity['id']]);$simulations=$statement->fetchAll();$active=$completed=$failed=[];foreach($simulations as $simulation){$simulation['state']=json_decode((string)$simulation['state'],true);$simulation['route_label']=Simulator::routeLabel($simulation['state']['route']??[]);$simulation['successful']=$simulation['status']==='completed'&&($simulation['state']['node']??null)==='portofino';if($simulation['successful']){if(isset($simulation['state']['finished_elapsed_seconds'])){$elapsed=max(0,(int)$simulation['state']['finished_elapsed_seconds']);$simulation['elapsed_label']=sprintf('%dh %02dm %02ds',intdiv($elapsed,3600),intdiv($elapsed%3600,60),$elapsed%60);$simulation['record']=$elapsed<92340;}$completed[]=$simulation;}elseif(in_array($simulation['status'],['completed','failed'],true))$failed[]=$simulation;else$active[]=$simulation;}
