...
$app->get('/simulator',function(Request $request,Response $response)use($pdo,$render,&$identity):Response{
    ...
    $statement=$pdo->prepare($sql);$statement->execute($admin?[]:[$identity['id']]);$simulations=$statement->fetchAll();foreach($simulations as &$simulation){$simulation['state']=json_decode((string)$simulation['state'],true);$simulation['route_label']=implode(' / ',array_map([Simulator::class,'routeName'],$simulation['state']['route']??[]));$simulation['successful']=$simulation['status']==='completed'&&($simulation['state']['node']??null)==='portofino';if($simulation['successful']){$elapsed=max(0,(new DateTimeImmutable($simulation['simulated_at']))->getTimestamp()-(new DateTimeImmutable($simulation['departure_at']))->getTimestamp());$simulation['elapsed_label']=sprintf('%dh %02dm %02ds',intdiv($elapsed,3600),intdiv($elapsed%3600,60),$elapsed%60);}}unset($simulation);
