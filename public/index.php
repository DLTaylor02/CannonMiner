...
$app->map(['GET','POST'],'/settings/game',function(Request $request,Response $response)use($pdo,$settings,$render,$csrf):Response{
    $message=null;if($request->getMethod()==='POST'){$csrf($request);$body=(array)$request->getParsedBody();$values=[];foreach(['simulator_crash_base_percent'=>1,'simulator_crash_120_percent'=>14,'simulator_crash_145_percent'=>14,'simulator_weather_percent'=>5,'simulator_flat_tire_percent'=>5,'simulator_mechanical_failure_percent'=>1,'simulator_headwind_percent'=>1,'simulator_tailwind_percent'=>1,'simulator_police_percent'=>5,'simulator_road_event_percent'=>5] as $key=>$default)$values[$key]=(string)max(0,min(100,(float)($body[$key]??$default)));$settings->save($values);$message='Game environment saved.';}
    $values=$settings->all();foreach(['simulator_crash_base_percent'=>1,'simulator_crash_120_percent'=>14,'simulator_crash_145_percent'=>14,'simulator_weather_percent'=>5,'simulator_flat_tire_percent'=>5,'simulator_mechanical_failure_percent'=>1,'simulator_headwind_percent'=>1,'simulator_tailwind_percent'=>1,'simulator_police_percent'=>5,'simulator_road_event_percent'=>5] as $key=>$default)$values[$key]??=(string)$default;
