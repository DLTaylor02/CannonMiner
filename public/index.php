...
$vehicleInput = static function (array $body): array {
    ...
    return['name'=>$name]+$values+['max_fuel_cells'=>max(0,min(20,(int)($body['max_fuel_cells']??0))),'max_load'=>$maxLoad];
};
...
$app->map(['GET','POST'],'/settings/game',function(Request $request,Response $response)use($pdo,$settings,$render,$csrf):Response{
    $probabilityDefaults=['simulator_crash_base_percent'=>1,'simulator_crash_120_percent'=>14,'simulator_crash_145_percent'=>14,'simulator_weather_percent'=>5,'simulator_flat_tire_percent'=>5,'simulator_mechanical_failure_percent'=>1,'simulator_headwind_percent'=>1,'simulator_tailwind_percent'=>1,'simulator_police_percent'=>5,'simulator_road_event_percent'=>5];$loadDefaults=['simulator_load_driver'=>200,'simulator_load_fuel_cell'=>200,'simulator_load_additional_spare'=>200,'simulator_load_cruising_tires'=>0,'simulator_load_cruising_tune'=>50,'simulator_load_radio_scanner'=>1,'simulator_load_radar_scanner'=>5,'simulator_load_radar_jammer'=>25];
...
$app->post('/settings/game/vehicles',function(Request $request,Response $response)use($pdo,$csrf,$vehicleInput,$vehicleImage,$deleteVehicleImage):Response{
    $csrf($request);$image=null;try{$vehicle=$vehicleInput((array)$request->getParsedBody());$image=$vehicleImage($request);$id='vehicle-'.bin2hex(random_bytes(8));$statement=$pdo->prepare('INSERT INTO simulator_vehicles(id,name,mpg_below_35,mpg_35_70,mpg_above_70,capacity,top_speed,tuned_top_speed,max_fuel_cells,max_load,image_path) VALUES (?,?,?,?,?,?,?,?,?,?,?)');$statement->execute([$id,$vehicle['name'],$vehicle['mpg_below_35'],$vehicle['mpg_35_70'],$vehicle['mpg_above_70'],$vehicle['capacity'],$vehicle['top_speed'],$vehicle['tuned_top_speed'],$vehicle['max_fuel_cells'],$vehicle['max_load'],$image]);$_SESSION['advisory']='Vehicle added.';}catch(Throwable $error){$deleteVehicleImage($image);$_SESSION['notice']=$error->getMessage();}return$response->withHeader('Location','/settings/game')->withStatus(302);
})->add($requireSuperadmin)->add($guard);
