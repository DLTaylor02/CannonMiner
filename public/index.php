...
$vehicleInput = static function (array $body): array {
    ...
    $maxLoad=(int)($body['max_load']??0);if($maxLoad<1||$maxLoad>100000)throw new RuntimeException('Maximum load must be between 1 and 100,000.');
    $lemon=(float)($body['lemon']??1);if(!is_finite($lemon)||$lemon<0||$lemon>100)throw new RuntimeException('Lemon must be a percentage between 0 and 100.');
    return['name'=>$name]+$values+['max_fuel_cells'=>max(0,min(20,(int)($body['max_fuel_cells']??0))),'max_load'=>$maxLoad,'lemon'=>$lemon];
};
...
$app->get('/simulator',function(Request $request,Response $response)use($pdo,$render,&$identity):Response{
    $sql="SELECT s.id,s.user_id,u.username,s.status,s.mode,s.departure_at,s.simulated_at,s.state,s.created_at FROM simulations s JOIN users u ON u.id=s.user_id ORDER BY CASE WHEN s.status NOT IN ('completed','failed') THEN 0 WHEN s.status='completed' AND s.state->>'node'='portofino' THEN 1 ELSE 2 END,CASE WHEN s.status='completed' AND s.state->>'node'='portofino' THEN NULLIF(s.state->>'finished_elapsed_seconds','')::double precision END ASC NULLS LAST,s.updated_at DESC LIMIT 100";
    $statement=$pdo->query($sql);$simulations=$statement->fetchAll();$canDeleteOthers=in_array($identity['role'],['admin','superadmin'],true);$active=$completed=$failed=[];foreach($simulations as $simulation){$simulation['state']=json_decode((string)$simulation['state'],true);$simulation['can_delete']=$canDeleteOthers||(int)$simulation['user_id']===(int)$identity['id'];$simulation['route_label']=Simulator::routeLabel($simulation['state']['route']??[]);$simulation['successful']=$simulation['status']==='completed'&&($simulation['state']['node']??null)==='portofino';if($simulation['successful']){if(isset($simulation['state']['finished_elapsed_seconds'])){$elapsed=max(0,(int)$simulation['state']['finished_elapsed_seconds']);$simulation['elapsed_label']=sprintf('%dh %02dm %02ds',intdiv($elapsed,3600),intdiv($elapsed%3600,60),$elapsed%60);$simulation['record']=$elapsed<92340;}$completed[]=$simulation;}elseif(in_array($simulation['status'],['completed','failed'],true))$failed[]=$simulation;else$active[]=$simulation;}
    return$render($request,$response,'simulator.twig',['active_simulations'=>$active,'completed_simulations'=>$completed,'failed_simulations'=>$failed,'csrf'=>$_SESSION['csrf']]);
})->add($guard);
...
$app->map(['GET','POST'],'/settings/game',function(Request $request,Response $response)use($pdo,$settings,$render,$csrf):Response{
    $probabilityDefaults=['simulator_crash_base_percent'=>1,'simulator_crash_120_percent'=>14,'simulator_crash_145_percent'=>14,'simulator_weather_percent'=>5,'simulator_flat_tire_percent'=>5,'simulator_headwind_percent'=>1,'simulator_tailwind_percent'=>1,'simulator_police_percent'=>5,'simulator_road_event_percent'=>5];$loadDefaults=['simulator_load_driver'=>200,'simulator_load_fuel_cell'=>200,'simulator_load_additional_spare'=>200,'simulator_load_cruising_tires'=>0,'simulator_load_cruising_tune'=>50,'simulator_load_tuned_up'=>0,'simulator_load_radio_scanner'=>1,'simulator_load_radar_scanner'=>5,'simulator_load_radar_jammer'=>25];
    $message=$error=null;$body=[];if($request->getMethod()==='POST'){$csrf($request);$body=(array)$request->getParsedBody();$values=[];foreach($probabilityDefaults as $key=>$default)$values[$key]=(string)max(0,min(100,(float)($body[$key]??$default)));foreach($loadDefaults as $key=>$default)$values[$key]=(string)max(0,min(100000,(int)($body[$key]??$default)));$names=[];$seen=[];foreach(preg_split('/\R/u',(string)($body['simulator_driver_names']??''))?:[] as $name){$name=trim($name);$key=strtolower($name);if($name===''||isset($seen[$key]))continue;$seen[$key]=true;$names[]=substr($name,0,120);if(count($names)>=500)break;}if(count($names)<6)$error='Enter at least six unique roster names, one per line.';else{$values['simulator_driver_names']=implode("\n",$names);$settings->save($values);$message='Game environment saved.';}}
    $values=$settings->all();foreach($probabilityDefaults+$loadDefaults as $key=>$default)$values[$key]??=(string)$default;$values['simulator_driver_names']=$error?(string)($body['simulator_driver_names']??''):($values['simulator_driver_names']??implode("\n",Simulator::DEFAULT_DRIVER_NAMES));
    return$render($request,$response,'game-settings.twig',['settings'=>$values,'vehicles'=>$pdo->query('SELECT * FROM simulator_vehicles ORDER BY name')->fetchAll(),'message'=>$message,'error'=>$error,'csrf'=>$_SESSION['csrf']]);
...
$app->post('/settings/game/vehicles',function(Request $request,Response $response)use($pdo,$csrf,$vehicleInput,$vehicleImage,$deleteVehicleImage):Response{
    $csrf($request);$image=null;try{$vehicle=$vehicleInput((array)$request->getParsedBody());$image=$vehicleImage($request);$id='vehicle-'.bin2hex(random_bytes(8));$statement=$pdo->prepare('INSERT INTO simulator_vehicles(id,name,mpg_below_35,mpg_35_70,mpg_above_70,capacity,top_speed,tuned_top_speed,max_fuel_cells,max_load,lemon,image_path) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)');$statement->execute([$id,$vehicle['name'],$vehicle['mpg_below_35'],$vehicle['mpg_35_70'],$vehicle['mpg_above_70'],$vehicle['capacity'],$vehicle['top_speed'],$vehicle['tuned_top_speed'],$vehicle['max_fuel_cells'],$vehicle['max_load'],$vehicle['lemon'],$image]);$_SESSION['advisory']='Vehicle added.';}catch(Throwable $error){$deleteVehicleImage($image);$_SESSION['notice']=$error->getMessage();}return$response->withHeader('Location','/settings/game')->withStatus(302);
})->add($requireSuperadmin)->add($guard);
